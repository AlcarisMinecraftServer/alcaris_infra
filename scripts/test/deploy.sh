...
VM_LIST=(
  ...
  "1001 alcaris-k8s 4    8192 192.168.0.11 192.168.0.110 nmkmn-srv-prox01"
)
...
runcmd:
  ...
  - su - cloudinit -c "curl -s ${REPOSITORY_RAW_SOURCE_URL}/scripts/test/setup.sh > ~/setup.sh"
  - su - cloudinit -c "sudo bash ~/setup.sh ${VMNAME} ${VMSRVIP}"
